...
export type DepthCarouselItem = string | { image: string; alt?: string; title?: string };
...
export interface DepthCarouselProps {
  ...
  onChange?: (index: number, item: { image: string; alt?: string; title?: string }) => void;
  className?: string;
}
...
const DEFAULT_ITEMS: DepthCarouselItem[] = [
  { image: '/aws.png', alt: 'AWS Certified Solutions Architect', title: 'AWS Certified Solutions Architect' },
  { image: '/ijfmr.png', alt: 'Research Paper Publication', title: 'Research Paper Publication' },
  { image: '/cyber.png', alt: 'Cyber Security Certification', title: 'Cyber Security Certification' },
  { image: '/hack.png', alt: 'National Hackathon Winner', title: 'National Hackathon Winner' },
  { image: '/devops.png', alt: 'DevOps & Cloud Engineering', title: 'DevOps & Cloud Engineering' }
];

const clamp = (v: number, min: number, max: number) => Math.min(Math.max(v, min), max);
const normalizeItem = (it: DepthCarouselItem) => (typeof it === 'string' ? { image: it, alt: '', title: '' } : it);
        ...
        {data.map((item, i) => (
          ...
          >
            <div className="relative w-full h-full bg-slate-950 flex items-center justify-center p-2">
              ...
              <img
                className="block max-h-full max-w-full select-none object-contain [pointer-events:none] [-webkit-user-drag:none]"
                src={item.image}
                alt={item.alt || ''}
                draggable={false}
              />
            ...
            {item.title && (
              <div className="absolute bottom-0 left-0 right-0 p-2.5 bg-gradient-to-t from-black/95 via-black/70 to-transparent text-white text-[11px] font-semibold text-center truncate z-10 border-t border-white/10">
                {item.title}
              </div>
            )}
            <span
