...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  return (
    <div
      className={`sm-scope z-40 ${isFixed ? 'fixed top-0 right-0 w-auto h-auto overflow-visible' : 'w-full h-full'}`}
      style={isFixed ? { pointerEvents: 'none', width: 'auto', height: 'auto' } : undefined}
    >
      <div
        className={(className ? className + ' ' : '') + 'staggered-menu-wrapper relative w-full h-full z-40'}
        style={{
          ...(accentColor ? ({ ['--sm-accent' as string]: accentColor } as React.CSSProperties) : {}),
          ...(isFixed ? { pointerEvents: 'auto' } : {})
        }}
